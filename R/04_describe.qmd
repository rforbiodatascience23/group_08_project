...
figure1 <- meta_data |>
  ...
  labs(x = "pCR Status",
       y ="Samples",
       title = "Samples per pCR status by Treatment",
...
The full_expression_data contains all the observations and variables that we need to use for the analysis.
...
In this figure, we illustrate the average gene expression count of all the samples by the treatment.

```{r, fig.height=6, fig.width=10}
#| echo: false
figure2 <- full_expression_data |>
  group_by(Sample) |>
  ...
  labs(x = "Samples",
       y = "Average Counts",
       title = "The average gene expression count of all the samples by the treatment",
       legend = "Treatment")
...
The 3. figure displays the differences in gene expression counts between the pre-and post-treatment by the genes. As an example, we arbitrary chose 1-1 pCR-patient from each treatment type and filtered the gene expression counts.
...
p1 <- full_expression_data |>
  ...
  labs(x ="Genes",
       y ="Total Counts",
       title ="A Pre-treatment Sample of pCR Patient_07 filterd by 500 > Counts")
...
p2 <- full_expression_data |>
  ...
  labs(x = "Genes",
       y = "Total Counts",
       title = "A Post-treatment Sample of pCR Patient_07 filterd by 500 > Counts")
...
The 4. figure displays the differences in gene expression counts between the pre-and post-treatment by genes. As an example, we arbitrary chose 1-1 no_pCR-patient from each treatment type and filtered the gene expression counts.
...
Our last figure illustrates the differences in gene expression counts between the no_pCR and pCR patients in each treatment type by genes.

```{r, fig.height=27, fig.width=27}
#| echo: false
p5 <- full_expression_data |>
  filter(Count > 2500 &
         Treatment == "pre_treatment") |>
  ggplot(aes(x = Gene,
             y = Count,
  ...
  labs(x ="Genes",
       y = "Total Counts",
       title = "Total Counts of the Genes before the treatment filtered by pCR_status")

p6 <- full_expression_data |>
  filter(Count > 2500 &
         Treatment == "post_treatment") |>
  ggplot(aes(x = Gene,
             y = Count,
  ...
  labs(x ="Genes",
       y = "Total Counts",
       title = "Total Counts of the Genes after the treatment filtered by pCR_status")
